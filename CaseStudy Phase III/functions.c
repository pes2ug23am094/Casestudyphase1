...
void generateBill(Customer *customers, int num_customers, Product *products, int num_products) {
    ...
    for (int i = 0; i < num_customers; i++) {
        if (customers[i].id == customer_id) {
            ...
            printf("=======================================================\n\n");
            customers[i].balance += total_bill; 
...
void saveCustomersToFile(Customer *customers, int num_customers) {
    ...
    fprintf(file, "FirstName | LastName | ID | PhoneNumber | TotalAmount\n");
...
void searchCustomer(Customer *customers, int num_customers) {
    ...
    printf("Enter customer name or phone number to search: ");
    scanf("%s", search_query);

    printf("===== Search Results =====\n");
    for (int i = 0; i < num_customers; i++) {
        if (strcmp(customers[i].first_name, search_query) == 0 || strcmp(customers[i].last_name, search_query) == 0 || strcmp(customers[i].phone_number, search_query) == 0) {
            printf("Name: %s %s, ID: %d, Phone: %s, Total Amount: %.2f\n", customers[i].first_name, customers[i].last_name, customers[i].id, customers[i].phone_number, customers[i].total_amount);
            found = 1;
        }
    }
    ...
    if (!found) {
        printf("No customer found matching the search query.\n");
    }
}
